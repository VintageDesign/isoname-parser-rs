...
fn parse_isoname(full_isoname: String) -> i32 {
    if let Ok(isoname_integer) = u64::from_str_radix(full_isoname.trim_start_matches("0x"), 16) {
        ...
        println!(
            "Address Arbitration Capable: \t{}",
            name.get_address_arb_capable()
        );
        println!("Industry Group: \t\t{}", name.get_industry_group());
        println!(
            "Vehicle System Instance: \t{}",
            name.get_vehicle_system_instance()
        );
        println!(
            "Vehicle System (Device Class): \t{}",
            name.get_vehicle_system()
        );
        println!("Function Code: \t\t\t{}", name.get_function_code());
        println!(
            "Function Instance: \t\t{}",
            name.get_function_code_instance()
        );
...
fn print_help() {
    println!(
        r"Usage: isoname-parser [0x<64-bit hex number>] [options]
Possible Options:
